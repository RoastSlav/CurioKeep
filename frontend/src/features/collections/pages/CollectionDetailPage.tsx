...
export default function CollectionDetailPage() {
    ...
    const filteredItems = useMemo(() => {
        ...
        return base.filter((item) => {
            ...
            return [title, item.id, identifiers].filter(Boolean).some((val) => val.toLowerCase().includes(q))
        })
    ...
    return (
        <div className="space-y-6">
            ...
            {!modules.length ? (
                <EmptyState title="No modules enabled"
                            description="Ask an admin to enable modules for this collection."/>
            ) : (
