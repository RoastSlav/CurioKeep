...
export default function WorkflowRunner({
    ...
    const content = useMemo(() => {
        if (!current)
            return (
                <Alert>
                    <AlertDescription>No steps defined for this workflow.</AlertDescription>
                </Alert>
            )
        switch (current.type) {
            case "PROMPT": {
                ...
                if (!field)
                    return (
                        <Alert variant="destructive">
                            <AlertDescription>Workflow references missing field {current.field}</AlertDescription>
                        </Alert>
                    )
                return <PromptStep field={field} values={attributes} onSubmit={handlePromptSubmit} onCancel={onCancel}/>
            }
            case "PROMPT_ANY": {
                const fields = (current.fields || []).map((key) => resolveField(key)).filter(Boolean) as NonNullable<
            ...
            default:
                return (
                    <Alert variant="destructive">
                        <AlertDescription>Unsupported step {String((current as any)?.type)}</AlertDescription>
                    </Alert>
                )
        }
    }, [
