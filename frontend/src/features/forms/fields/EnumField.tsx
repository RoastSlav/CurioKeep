    ...
    return (
        <div className="flex flex-col gap-2">
            ...
            <Select value={current} onValueChange={onChange} disabled={disabled}>
                <SelectTrigger
                    id={id}
                    className={`brutal-border ${error ? "border-destructive" : ""} bg-card text-card-foreground`}
                    onBlur={onBlur}
        >
                ...
                <SelectContent className="brutal-border bg-card">
                    {(field.enumValues || []).map((opt) => (
                        <SelectItem key={opt.key} value={opt.key} className="text-card-foreground hover:bg-muted">
                            {opt.label || opt.key}
                        </SelectItem>
                    ))}
                </SelectContent>
